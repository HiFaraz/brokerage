...
function provider(token) {
	...
	interface.quotes = function (id) {
		if (Array.isArray(id)) id = 'ids=' + id.join(','); // TODO this really depends on if the provider supports many ids at once. If it doesn't, this will have to translate into multiple calls
		return new Promise(
			function (resolve, reject) {
				get('markets/quotes/' + id).then(function (response) {
					var quotes = response.entity.quotes.map(symbolAdapter);
					if (quotes.length == 1) quotes = quotes[0];
					resolve(quotes);
				}, fail(reject));
			}
		)
	...
	interface.symbol = function (id) {
		return new Promise(
			function (resolve, reject) {
				get('symbols/' + id).then(function (response) {
